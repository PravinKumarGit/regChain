...
var _illinois_counties_Arr_110 = ['610'];
	...
	*/
	...
	// Commented by lovdeep as per discussion with vinod sir on 08-06-2018
	...
	// Added by lovdeep as per discussion with vinod sir on 08-06-2018
	...
	export function getAdjustedVA(request) {
		...
		downPayment = parseFloat(request.salePrice) - parseFloat(amount);
	...
	*/
	export function getDiscountAmount( request ){
		...
		//console.log("getDiscountAmount discount " + discount);
	...
	*/
	...
	export function getOriginationFee(request) {
		...
		if (request.originationFactorType == "%") {
			console.log("if condition request.originationFactorType == '%' ");
			originationFee = parseInt(request.amount) * parseInt(request.originationFee) / 100; //Formula applied here to calculate the origination Fee
			console.log("originationFee " + originationFee);
		} else {
			console.log("else condition request.originationFactorType == '%' ");
...
export function getBuyerEstimatedTax(request) {
	...
	if (stateCheck != false && countyCheck !== false) { // state - '23','10','36','39' // for mobile
		...
		//firstQuarter_perDayTax = Math.floor(firstQuarter_perDayTax * 100) / 100;
		//secondQuarter_perDayTax = Math.floor(secondQuarter_perDayTax * 100) / 100;	
		summerTaxProrations = (firstQuarter_perDayTax * summer_daysleftinmonth);
		winterTaxProrations = (secondQuarter_perDayTax * winter_daysleftinmonth);
		...
		if (request.state_id == 32 || request.state_id == 17 || request.state_id == 26) {  // New Maxico
			...
			var daysleftinmonth = getIllinoisEstimatedTaxProration(request.county_id, current_date);
			daysleftinmonth = parseFloat(daysleftinmonth).toFixed(2);

			if(request.type == "buyer") {
				daysleftinmonth = 0 - daysleftinmonth;
			} 
			//console.log("daysleftinmonth " + daysleftinmonth);
		...
		console.log("perday tax " +  perDayTax);

		perDayTax = perDayTax * 100 / 100;
		perDayTax = Math.floor(perDayTax * 100) / 100;
		estimatedTaxProrations = (perDayTax * parseFloat(daysleftinmonth));
	
		console.log("estimatedTaxProrations " +  estimatedTaxProrations);
