...
var _illinois_counties_Arr_110 = ['610'];
	...
	*/
	...
	// Commented by lovedeep as per discussion with vinod sir on 07-25-2018
	/*
	...
	*/
		...
		// added by lovedeep as per discussion with vinod sir on 07-25-2018
	export function getSellerCostTypeTotal(request) {
		...
		if (request.type == '% Loan') {
		  if(request.loanType == "FHA" || request.loanType == "VA" || request.loanType == "USDA"){
			totalCostRate = request.rate * request.adjusted / 100;
		  } else {
			totalCostRate = request.rate * request.amount / 100;
		  }
		}
	...
	*/
	...
	export function getSellerListSellAgtValues(request){
        ...
        if(request.state == 'OH'){
			...
			// new change added by lovedeep as per discussion with vinod sir. here we are handling both case (traidtional as well as tiered). that's why we need to check if request is for tiered or for traditional
			if(request.commisionOption == 'tiered') {
				amount      = request.brokageFee;      
				list_agt    = request.listAgt * amount / 100;
				sell_agt    = request.sellAgt * amount / 100;	
			} else {
				list_agt    = request.listAgt * request.salePrice / 100;
       	 		sell_agt    = request.sellAgt * request.salePrice / 100;
			}
		} else if(request.state == 'HI'){
			amount  = request.salePrice;  
			list_agt = request.listAgt * amount / 100;
			sell_agt = request.sellAgt * amount / 100;
		 
			if(request.countyId == 547){
			 list_agt = list_agt + (list_agt * 4.712) / 100; 
			} else {
			 list_agt = list_agt + (list_agt * 4.166) / 100;      
			}
		 
			if(request.countyId == 547){
			 sell_agt = sell_agt + (sell_agt * 4.712) / 100; 
			} else {
			 sell_agt = sell_agt + (sell_agt * 4.166) / 100;      
			}
...
export function getSellerEstimatedTax(request) {
	...
	} else if (request.state_id == 54234) {  // // CA State not in use		
		if (parseFloat(prorations) > 0) {
			var daysleftinmonth = Math.abs(prorations) - dated; // Debit - proration in positive case
		} else if (parseFloat(prorations) == '0.00') {
			var daysleftinmonth = 0;
	...
	if (stateCheck != false && countyCheck !== false) { // state - '23','10','36','39' // for mobile
		...
		//firstQuarter_perDayTax = firstQuarter_perDayTax * 100 / 100;
		//secondQuarter_perDayTax = secondQuarter_perDayTax * 100 / 100;	
		...
		//perDayTax = perDayTax * 100 / 100;
		//perDayTax = Math.floor(perDayTax * 100) / 100;
...
function getIllinoisEstimatedTaxProration(countyIdHiddenValue, settlement_date) {
	...
	*/
	...
	export function getSellerListSellAgtPer(request){      
        ...
        if(request.state == 'OH'){
            amount  = request.brokageFee;       
            listAgt = request.list_agt / amount * 100;
            sellAgt = request.sell_agt / amount * 100;
        } else { 
            listAgt = request.list_agt / request.salePrice * 100;
            sellAgt = request.sell_agt / request.salePrice * 100;
        }
	...
	*/
	...
	export function getSellerListSellTeired(request){
